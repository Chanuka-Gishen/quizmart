...
backendAuthApi.interceptors.request.use((request) => {
  const bearerToken = reduxPersistStore.getState().auth.admin.token;
  if (bearerToken) {
  ...
  (error) => {
    if (error && !axios.isCancel(error)) {
      ...
      if (error.response) {
         ...
         */
        if (
          !common_util.isUndefinedOrNull(errorResponse.responseCode) &&
          (errorResponse.responseCode === 'AUTH-002' ||
            errorResponse.responseCode === 'AUTH-003' ||
            errorResponse.responseCode === 'AUTH-004')
        ) {
          errorMessage = SNACKBAR_MESSAGE.SESSION_TIMEOUT.MESSAGE;
